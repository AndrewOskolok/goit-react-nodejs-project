...
async function createTransaction(req, res) {
    ...
    const userToUpdate = await UserModel.findByIdAndUpdate('5f85f5c86e965b45cca0574e', { $push: { transactions: { ...req.body, id: transactionId } } },)
...
async function deleteTransaction(req, res) {
    const userToUpdate = await UserModel.findByIdAndUpdate('5f85f5c86e965b45cca0574e', { $pull: { transactions: { id: `${req.params.transactionId}` } } })
    // Вместо первого параметра будет браться ID авторизированного юзера из объекта request.
    if (!userToUpdate) {
        return res.status(404).send({message: "Unauthorized"})
    }
